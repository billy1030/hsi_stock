...
- Always compile `hsi_stock_go` using the new standard method: `wails build -clean -trimpath` (in `c:\ai\etnet\hsi_stock_go`).
...
- Always stop any running `HSI_Stock.exe` process before building/copying to avoid process lock conflicts.
